...
class UserTest < ActiveSupport::TestCase
  def setup #при запуске теста создается экземпляр класса
  ...
  test "email should be present" do
    @user.email = "a"*256 + "@user.com"
    assert_not @user.valid?
  end

  test "email validation should accept valid addresses" do
    valid_addresses = %w[example@user.com USER@foo.COM A_US-ER@foo.bar.org first.last@foo.jp alice+bob@baz.cn]
